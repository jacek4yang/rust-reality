...
| D9 | supported-not-integrated | Framed path is AEAD-bound; ring/OpenSSL ≈2–2.5× RustCrypto at production record sizes | SUPPORTED (isolated): decomposition download AEAD 51%/kernel 47%, upload 39%/57% (perf, d28c5f0, benchmarks/final/framed-prof/); crypto-bench AES-128-GCM 16KiB: ring 5.16 / OpenSSL 4.08 / RustCrypto 2.03 GiB/s (ring 2.54×, in-place, conservative for OpenSSL). Amdahl ceiling 1.45× download / 1.31× upload via ring. Copy-elimination REJECTED (0 avoidable copies, COPY-MAP.md); Vision/record-parse <1%; scheduler ≈1%. Integration is a supply-chain product decision (ring embeds BoringSSL C/asm; SECURITY.md documents pure RustCrypto) — flagged to user before any production change. |
