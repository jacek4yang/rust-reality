...
| D7 | resolved-deleted | Sockhash pair-path unreachable after c1ec2cf → delete privileged code | DELETED: zero sockhash arms across all production benchmark matrices; privileged A/B parity with splice (c1 1642 vs 1637, c4 3086 vs 3109, c32 3245 vs 3282 MiB/s); the unprivileged production deployment model can never arm it. Backend, eBPF program/verifier/attach path, config knobs (`sockhash`, `maxSockhashRelays`, `maxPinnedMemoryBytes`), privileged gates and A/B script removed (~5400 lines); automatic order is now splice → buffered. Retained evidence: benchmarks/final/sockhash-ab/. |
...
| D9 | PROVEN | Framed path is AEAD-bound; ring ≈2.5× RustCrypto at production record sizes | PROVEN (859115e): byte-exact provider equivalence + RFC 8448 under both configs; E2E framed 1.00–1.18× across all 16 cells (512MiB: 1.05–1.16×), ≥Xray in all four 512MiB cells; server CPU/GiB −33%, instructions −30%; RSS +3% noise; attribution: RustCrypto AEAD symbols (~50%) replaced by ring AVX2 GCM asm, userspace share 49.2→37.0%. Below the 1.44×/1.31× Amdahl ceiling because loopback throughput is host-CPU-shared. Supply chain: zero new crates (ring already via ureq/rustls), deny/audit pass, static link, −13KiB binary. Full report: reports/D9-RING-AEAD-EVIDENCE.md. KEPT as opt-in feature; default flip recommended after zeroization tradeoff + SECURITY.md wording are decided. |
