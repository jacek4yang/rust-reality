...
对比 Xray：同一 perf 归因 setup 基准中服务端每建连 CPU 为 609 µs 对
988 µs——rust-reality 完成一次 VLESS + REALITY + Vision 建连的服务端
CPU 约为 Xray 的 0.62×。
...
- 建连 CPU 成本：每连接服务端 CPU 约为 Xray 的 0.62×（609 µs 对
  988 µs task-clock，perf 归因）。
