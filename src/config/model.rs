...
pub struct RuntimeConfig {
    ...
    ///
    /// `shared` resolves to the standard posture and `dedicated` to the
    /// dedicated posture. `auto` (the default) detects single-tenancy from
    /// the cgroup v2 boundaries — it never guesses dedicated on bare metal.
    #[serde(default)]
...
impl RuntimeConfig {
    /// Resolves the effective resource mode from `profile` and the detected
    /// machine view.
    ///
    /// A non-`auto` profile maps onto the mode, and `auto` resolves to
    /// `dedicated` only when the cgroup v2 tenancy boundary is fully
    /// observable (a finite `cpu.max` quota and a finite `memory.max`);
    /// `auto` never guesses dedicated on bare metal.
    #[must_use]
    pub fn resolve_resource_mode(
        &self,
        machine: &crate::runtime::machine::MachineReport,
    ) -> ResourceMode {
        match self.profile.resource_mode() {
            Some(mode) => mode,
            None => {
    ...
    ///
    /// Both resource modes are resolved against the same machine view, so a
    /// profile flip that changes the resolved mode rejects; `profile` and
...
pub enum RuntimeProfile {
    /// Detect single-tenancy from the cgroup v2 boundaries; never guesses
    /// dedicated on bare metal.
    #[default]
    Auto,
    /// Shared machine; resolves to the standard resource mode.
    Shared,
    /// This process owns the machine or cgroup; resolves to the dedicated
    /// resource mode.
    Dedicated,
}
...
#[cfg(test)]
mod tests {
    use super::{Objective, ResourceMode, RuntimeConfig, RuntimeProfile, TuningConfig, TuningMode};
