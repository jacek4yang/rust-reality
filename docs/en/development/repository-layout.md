...
| `crates/rr-linux/` | The Linux OS/ABI boundary: sockets, pipes and `splice`, rlimits, socket options, `/proc` memory sampling. `#![no_std]`; mechanisms go through `rustix`'s `linux_raw` backend and report `Errno`, and descriptors leave as `OwnedFd`. Isolated so the main crate can stay `#![deny(unsafe_code)]`. |
