...
| Harness | Purpose |
|---|---|
| `cargo bench` (criterion, in `benches/`) | Bounded in-process protocol measurements: VLESS request decode, both Vision decode paths (`8k_single_fragment` stages the payload into caller storage, `16k_borrowed` is the steady-state relay path that borrows the record without copying), and the NXR authenticated request encode. Optimising against a case the production session does not execute is the failure this split exists to prevent. |
...
| `cargo dev bench run --suite setup-rate` | Balanced setup-rate A/B (accept → first Vision transition) between a pinned baseline ELF and the candidate. `--cover-netem-rtt-ms` moves only the TLS cover behind a veth/netns and applies a recorded one-leg delay, retaining pool hit/miss summaries. `--measure-mode perf` attributes task-clock/instructions/context switches after warmup; `schedstat` attributes the same server CPU from `/proc/<pid>/task/*/schedstat` on a host with no `perf` binary, carrying no instruction or context-switch counts and bounding what a thread exiting mid-window could hide; `strace` records the bounded read/receive syscall set and gracefully stops the tracee so summaries cannot be silently empty. `--profile` captures the first measured candidate slot under the benchmark's existing lock and exact server identity. |
| `cargo dev bench run --suite vision-direct`, `cargo dev bench run --suite xray` | Focused Vision-Direct and Xray comparisons. Vision-Direct enables the debug-level structured events its tunnel guard consumes and fails if the measured downloads produce no completed Direct server path. `--profile` is available for Vision-Direct and samples the exact Rust server across the post-warmup measured workload. |
...
| `cargo dev perf freeze` | Builds one named commit into an identity-bound, read-only release artifact that every other perf command can consume. Rust owns commit-format bounds, the refusal to build anything but that exact commit from a clean worktree, embedding the commit through `RUST_REALITY_GIT_COMMIT`, read-only archival with post-copy re-verification, and the requirement that the built binary report the requested commit back. A forgotten commit variable therefore fails at freeze time instead of inside a capture minutes later. The full build log stays in the evidence directory, `freeze.json` is the machine record, and the printed summary is a compact projection of both. It never moves `HEAD` on the caller's behalf. |
| `cargo dev perf hotspot` | Identity-bound `perf record` capture attached to an existing, exactly identified server PID. Rust owns argument bounds, exact PID/start-time/executable identity, a deadline that ends cleanly when either the target or requested interval finishes, read-only binary archival, report/build-ID checks, checksums, publication, and cleanup; the archived report is rendered with the same privilege the record required, so kernel frames resolve to symbols rather than raw addresses. `timeout`, `perf`, `readelf`, and `sudo` remain typed external mechanisms. |
| `cargo dev perf hotspot-bundle` | Exports one `--dso-offset` from a completed native hotspot run through executable ELF `PT_LOAD` normalization, DWARF, IDALib, LLVM and exact perf DSO-offset samples. Rust owns the completed-capture identity, file-offset-to-static-address conversion, private binary/database paths, IDALib result validation, function selection, instruction aggregation, row/period conservation, the zero-default/sub-1% mapping gate, checksums, failure state and publication. This works for stripped PIEs without treating perf's file offset as an ELF virtual address or trusting a synthetic IDA function name as a perf symbol. IDALib's Python-only automation API is isolated behind a generated private direct-API bridge; the operator supplies `--idalib-python` plus any installation environment, and no repository Python policy file or machine-specific installation path remains. |
