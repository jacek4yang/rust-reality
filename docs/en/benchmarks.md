...
| Harness | Purpose |
|---|---|
| `rust-reality benchmark` (built-in) | Bounded, machine-readable in-process protocol measurements. Both VLESS request decoders are measured: `vless.decode.ipv4` is the owned public API, `vless.decode.ipv4.borrowed` is the parser a real session runs. Both Vision decode paths are measured too: `vision.decode.8k` stages the payload into caller storage, while `vision.decode.8k.borrowed` is the steady-state relay path that borrows the record without copying. Optimising against a case the production session does not execute is the failure this split exists to prevent. `nxr.auth.encode.domain` covers NXR authentication. |
