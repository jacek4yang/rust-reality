...
### 配对 block 数量，以及一个区间可以声称什么

配对套件（`setup-rate`、`fallback`）用平衡 ABBA block 比较两个构建，并用确定性
95% block bootstrap 汇总每个 cell。bootstrap 重采样的是 **block**。当 block 数
低到一定程度时，它们根本不足以描述抽样分布，区间会退化成大致等于观测到的那几个
block 的极差——同时看上去仍像一个很紧的置信界。

因此 `--blocks` 的默认值改为发布评估器早已强制的同一个下限（12 个完整 block，见
[ADR 0022](../adr/0022-paired-block-resolution.md)）。低于该值时，cell 发布
`bootstrap95Unresolved` 与 `resolutionCaveat`，而不是 `bootstrap95`，使低分辨率
区间无法被归档、引用或按“已解析”解析。每个 cell 还带上 `blockCount` 和
`blockRatioSpread`（逐 block 比值的最小值、最大值与标准差）——因为决定一个亚百分点
的中位差异是否能与该次运行自身噪声区分开的，是这个离散度，而不是区间。摘要记录
`runKind`，两侧为同一制品时取值 `control`，因此同二进制 A/A 运行在归档中自证身份。

**这个下限是对着什么测出来的。** 实验室主机（i5-1240P，用 `taskset -c 0,2,4,6`
钉住四个 P 核，`--measure-mode schedstat`，两侧同一个冻结 ELF，共 20 次运行）上的
同二进制 A/A 阶梯，指标为 `serverCpuPerConnection`：

| block | A/A 运行数 | 报出的区间不含 1.0 | 中位比值 | 报出的区间宽度 |
| ---: | ---: | ---: | --- | --- |
| 3 | 14 | **4** | 0.9968 – 1.0107 | 0.27% – 2.00% |
| 12 | 4 | **0** | 0.9987 – 1.0019 | 0.57% – 0.79% |
| 20 | 2 | **0** | 1.0032 – 1.0043 | 0.91% – 1.51% |

十四次同二进制运行中有四次在三 block 下被判为显著不同，而名义水平是 5%。宽度这一
列更能说明问题：三 block 下报出的宽度反映的是三次抽样的运气，而不是该次运行真正的
不确定度，于是一次运行可以既报出 ±0.13% 又排除 1.0，而另一次完全相同的运行报出
±1%。到十二个 block 时宽度稳定在 0.7% 附近，并且确实跟随它所来自的离散度。完整
证据与被否决的替代方案见 [ADR 0022](../adr/0022-paired-block-resolution.md)。

默认值的代价很小，也应当写明：在该主机上 `setup-rate` 形状下，三个 block 约 56 秒，
十二个约三分半。刻画性运行仍可传入更小的 `--blocks`，其产物会说明自己是什么。
...
| `cargo dev bench run --suite setup-rate` | 在固定基线 ELF 与候选之间做平衡 setup 速率 A/B（accept → 第一次 Vision 转换）。设置 `--cover-netem-rtt-ms` 时只把 TLS 伪装目标移到 veth/netns 后并施加有记录的单向延迟，同时保留 pool hit/miss 汇总。`--measure-mode perf` 在 warmup 后归因 task-clock/指令/context switch；`schedstat` 在没有 `perf` 二进制的主机上，从 `/proc/<pid>/task/*/schedstat` 归因同一份 server CPU，它不携带指令数与 context switch 计数，并对窗口内退出的线程可能隐藏的时间给出上界；`strace` 记录有界的 read/receive syscall 集，并先优雅停止 tracee，避免静默产生空汇总。`--profile` 在 benchmark 已持有的锁和精确 server 身份下采集第一个正式候选 slot。 |
| `cargo dev bench run --suite vision-direct`、`cargo dev bench run --suite xray` | 聚焦的 Vision-Direct 与 Xray 对比。Vision-Direct 启用 tunnel 守卫所依赖的 debug 级结构化事件；如果测量下载未产生已完成的 Direct server 路径，它会失败。Vision-Direct 支持 `--profile`，对 warmup 之后的完整测量 workload 采样精确的 Rust server。 |
...
| `cargo dev perf hotspot` | 对既有且已精确识别的 server PID 进行身份绑定的 `perf record` 采集。Rust 负责参数边界、精确 PID/start-time/可执行文件身份、在目标或请求时间结束时正常停止的截止机制、只读二进制归档、report/build-ID 校验、校验和、发布与清理；归档报告以 record 所需的同等权限生成，因此内核栈帧解析为符号而不是裸地址。`timeout`、`perf`、`readelf`、`sudo` 仅作为带类型 argv 的外部机制。 |
...
`23d228d7…04c5268`）。 v1.9.0 **没有跑新的对比活动**，这一节存在的意义就是把这件事说清楚，
而不是暗示跑过。

v1.9.0 替换了运维配置 schema、CLI 界面和公开文档。它没有改动任何线路字节，也没有
改动任何数据路径。唯一触及每连接路径的改动是在减少工作而不是增加：`network` 和
`inboundTag` 这两个路由条件恒为真，它们连同求值守卫一起被删除，所以现在一次路由
决策求值的谓词严格少于 v1.8.0。

因此 v1.8.0 的测量基础连同其已声明的局限一并保持不变。支撑它的门禁输入与结论见
[performance.zh-CN.md](performance.md#v190-发布证据)。

stock Xray 兼容性门禁所证明的内容没有变化，其测试装置针对新的 CLI 和 schema 做了
修复：它现在自己组装 rust-reality 配置，而不是去调用一个已经不存在的生成器。ML-DSA
差分检查随着它背后的命令一起被移除。

下方 v1.8.0 及更早的头条表格仍是测量基础，未作改动。
