...
`23d228d7…04c5268`）。 v1.9.0 是配置与文档发布。它的性能主张是**中性**，而这个主张的依据是
"改了什么"，不是一次新的测量活动。

没有数据路径被修改。本次发布中的模块拆分挪动的是冷控制面——启动、代次发布、热更新、
监听器监管，以及策略推导——全部发生在同一个 crate 内部；没有函数跨越 crate 边界，
没有派发变成动态派发，也没有任何连接会经过的签名发生变化。

有两处改动触及每连接路径，而且两处都在减少工作：

- `network` 和 `inboundTag` 这两个路由条件，对这个服务端接受的任何配置来说都恒为真。
  它们连同求值守卫一起被删除，所以一次路由决策求值的谓词严格更少。
- 有效策略不再被写回配置对象，因此重载不再重新推导并重新比较它。那属于控制面而不是
  数据路径，但这正是重载比较现在变成结构性比较的原因。

被移除的基准输入在取值上是等价的：`StartupPlan::derive` 去掉了 `Probes` 参数，而每个
调用方传的都是 `Probes::default()`，它喂给的算术在那个取值下是恒等的——握手池和直连
速率上的 `.max(0)`，以及转发缓冲区的 32 KiB 常量。`runtime/plan/tests.rs` 里的黄金
向量把结果钉住了。

凡是需要测量才能成立的主张，本次一概不作。这次发布没有跑 v1.8.0 跑过的四门禁
`cargo dev perf evaluate` 活动，因为没有任何待判定的数据路径改动。

## v1.8.0 发布证据

v1.8.0 是架构发布：不改变任何 wire 字节、配置结构或部署身份，其性能主张是
**中性**而非提升。
...
| 分阶段改动 | 受保护指标数 | 结论 |
|---|---:|---|
| 会话引擎抽取（`rr-session`） | 并发 1 下 32 项，另有覆盖并发 1/8/32 的 24 项 | PASS，0 回归 |
| 用类型证明不可逆写边界 | 32 | PASS，0 回归 |
| 显式 Tokio 运行时适配层 | 32 | PASS，0 回归 |
| 传输能力边界 | 32 | PASS，0 回归 |

每连接服务端 CPU 是这些改动最可能影响的指标，四个门禁中都落在噪声范围内：
1.0051 [0.9952, 1.0122]、0.9992 [0.9936, 1.0061]、1.0018 [0.9962, 1.0042]、
1.0029 [0.9924, 1.0094]。

代码体积是机械层面的交叉验证。相同编译参数下的 release tier 构建，第一个分阶段
改动的 `.text` 为 6 089 575 字节，其余三个为 6 089 319 字节：写边界改动通过删除
...
- **真实 IPv6 验证。** 原生 `cargo dev bench run --suite ipv6` 门禁在真实全球 IPv6 与真实
