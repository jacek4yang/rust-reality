...
| `runtime.profile` | no | `auto`; `auto`, `shared`, `dedicated` | Who owns this machine. `shared` maps onto `resourceMode: standard` and `dedicated` onto `resourceMode: dedicated`; declaring one together with a contradicting `resourceMode` is a validation error. `auto` defers to `resourceMode` when set and otherwise keeps the `standard` posture in this version; cgroup-based single-tenancy detection arrives with the startup-derivation slice. |
