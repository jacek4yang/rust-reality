...
2. **REALITY。** ClientHello 在有界时限内读取，要么认证成功，要么进入
   ...
   发出一条空的加密 ApplicationData 假 NST，占用服务端应用序号 0。因此
   established 服务端记录层按已认证计划从序号 0 或 1 开始；fallback 会收到
   每一个已检查字节。
...
服务。边界是精确的：路由已解析、尚未向客户端写入 Vision 响应、服务器方向的
记录序号按可选假 NST 为 0 或 1、尚不存在任何 Vision 状态。属主规则是**任意时刻一个会话
只有一个协议属主**：
