...
| `scripts/soak-test.sh` | Loopback mixed-workload soak (tunnel traffic + connection churn) with long-lived warm Handoff, NXR, and TCP-only SOCKS5 topologies, an atomic midpoint generation reload/reconnect, and per-process `/proc` leak bounds; env: `DURATION_MIN`, `ROUND_SLEEP`, `RUST_REALITY_BIN`, `XRAY_BIN`, `OUT_DIR`. |
