...
| `cargo dev perf hotspot-bundle` | Exports one address from a completed native hotspot run through DWARF, IDALib, LLVM and exact perf symbol offsets. Rust owns the completed-capture identity, private binary/database paths, IDALib result validation, instruction aggregation, the zero-default/sub-1% mapping gate, checksums, failure state and publication. IDALib's Python-only automation API is isolated behind a generated private direct-API bridge; no repository Python policy file remains. |
