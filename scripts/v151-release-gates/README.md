# v1.5.1 release gates

Wrappers and analysis for the v1.5.1 release-gate evidence runs, mirroring
`scripts/v150-release-gates/`.  Baseline is the published **v1.5.0 release
asset** (gitCommit `eda773b`, SHA-256
`344a9d8f0d270115284d8872a12fa911b38c5db49e14efaa3e3682e78e19bbc9`, extracted
read-only at `artifacts/v1.5.1/baseline/extracted/rust-reality` and bound by
`baseline-identity-eda773b.json`).  The candidate is a **prebuilt read-only
binary passed via `RR_CANDIDATE_BIN`** because the v1.5.1 candidate commit is
not merged when this harness lands; its source commit defaults to the HEAD of
`REPOSITORY` (`worktrees/v151-gates`).  Xray comparator:
...
- `GATES=artifacts/v1.5.1/gates`, `TMPDIR=$GATES/tmp` (create it first),
- `RR_HOST_EXCLUSIVE_LOCK=/tmp/v151-bench.lock` — overrides
...
`/tmp/v151-bench.lock` serializes ALL benchmark/perf/soak runs on this host
across workers.
...
- **Formal contract runs** (setup-abba, fallback-abba, matrix-formal, and any
  ...
  `flock -x /tmp/v151-bench.lock` — the keeper opens its own FD on the same
...
- **Exploratory runs** (`EXPLORATORY=1`: run-matrix.sh, run-soak.sh,
  ...
  `flock -x /tmp/v151-bench.lock <cmd>`, hold it as briefly as possible, and
  verify afterwards that no orphan inherited it:
  `fuser -v /tmp/v151-bench.lock; pgrep -af 'bench-origin|rust-reality serve|xray'`.

## Invocations

```bash
source /home/jacek/work/kimi-rust-reality-performance/proxy-env.sh  # crates.io/GitHub only
mkdir -p /home/jacek/work/kimi-rust-reality-performance/artifacts/v1.5.1/gates/tmp
export RR_CANDIDATE_BIN=<read-only v1.5.1 candidate rust-reality ELF>
G=scripts/v151-release-gates

# Formal legs (contract lock via keeper; clean repo at worktrees/v151-gates):
$G/run-setup-abba.sh 01
$G/run-fallback-abba.sh 01
$G/run-matrix-formal.sh 01

# Exploratory legs (outer lock required):
flock -x /tmp/v151-bench.lock $G/run-matrix.sh 01
flock -x /tmp/v151-bench.lock env ABBA_START=final $G/run-matrix.sh 02
flock -x /tmp/v151-bench.lock $G/run-soak.sh baseline 01
flock -x /tmp/v151-bench.lock $G/run-soak.sh candidate 01
flock -x /tmp/v151-bench.lock $G/cpu-per-gib.sh \
    artifacts/v1.5.1/baseline/extracted/rust-reality baseline \
    artifacts/v1.5.1/gates/cpu-base-r01 61600
flock -x /tmp/v151-bench.lock $G/cpu-per-gib.sh \
    "$RR_CANDIDATE_BIN" candidate artifacts/v1.5.1/gates/cpu-cand-r01 61610
...
python3 scripts/evaluate-release-performance.py \
    artifacts/v1.5.1/gates/evaluator-manifest.json
python3 $G/analyze-gates.py   # exploratory c32 matrix + cpu-per-gib; exit 1 on regression
```

## README comparison gap cells (G1/G2/G3/G5, notes/v1.5.1/readme-plan.md)
...
```bash
S=$PWD/scripts   # from the v151-gates worktree (or any checkout at the candidate commit)
X=/home/jacek/work/kimi-rust-reality-performance/artifacts/xray-reference-v26.7.28
B=/home/jacek/work/kimi-rust-reality-performance/artifacts/v1.5.1/baseline/extracted/rust-reality

# G1 — setup rate / CPU-per-conn, Xray as SERVER leg vs rust server:
flock -x /tmp/v151-bench.lock env EXPLORATORY=1 \
...
`/tmp/v151-bench.lock` and no outer `flock` may be used.
