...
# Same cell plan as the v1.5.0 gate matrix (PAYLOADS "1 32",
# CONCURRENCIES "1 32", SAMPLES=5, INTEGRITY_MIB=1024) but v1.5.0-release
# baseline vs v1.5.1 release candidate.  EXPLORATORY=1 because the formal
# matrix contract demands fs.pipe-user-pages-soft >= ~344k pages at c32 and
# writing that sysctl is forbidden on this host;
# MANAGE_PIPE_USER_PAGES_SOFT=0 is pinned regardless.  Round 2 should run
# with ABBA_START=final to balance cross-round drift.
set -Eeuo pipefail
